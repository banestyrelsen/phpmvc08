 ...
 */
$asf->config['debug']['asdf'] = false;
$asf->config['debug']['session'] = false;
$asf->config['debug']['timer'] = true;
$asf->config['debug']['db-num-queries'] = true;
$asf->config['debug']['db-queries'] = true;


/**
 * Set database(s).
 */
$asf->config['database'][0]['dsn'] = 'sqlite:' . ASDF_SITE_PATH . '/data/.ht.sqlite';
 ...
 */
$asf->config['url_type'] = 1;


/**
 * Set a base_url to use another than the default calculated
 */
$asf->config['base_url'] = null;


/**
 * How to hash password of new users, choose from: plain, md5salt, md5, sha1salt, sha1.
 */
$asf->config['hashing_algorithm'] = 'sha1salt';


/**
 * Allow or disallow creation of new user accounts.
 */
$asf->config['create_new_users'] = true;


/**
 * Define session name
 */
$asf->config['session_name'] = preg_replace('/[:\.\/-_]/', '', __DIR__);
$asf->config['session_key']  = 'asdf';


/**
 * Define default server timezone when displaying date and times to the user. All internals are still UTC.
 */
$asf->config['timezone'] = 'Europe/Stockholm';


/**
 * Define internal character encoding
 */
$asf->config['character_encoding'] = 'UTF-8';


/**
 * Define language
 */
$asf->config['language'] = 'en';
 ...
 * Define menus.
 *
 * Create hardcoded menus and map them to a theme region through $asf->config['theme'].
 */
$asf->config['menus'] = array(
  'navbar' => array(
 ...
 * Template files can reside in the parent or current theme, the CAsdf::ThemeEngineRender()
 ...
 *
 * path: Path to current theme, relativly ASDF_INSTALL_PATH, for example themes/grid or site/themes/mytheme.
